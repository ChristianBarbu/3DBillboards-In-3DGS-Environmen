...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        [SerializeField] string m_InputFile2;
        ...
        string m_PrevPlyPath2;
        int m_PrevVertexCount;
        int m_PrevVertexCount2;
        long m_PrevFileSize;
        long m_PrevFileSize2;
        
        bool m_GenerateSuccessiveTrainedAlignedAssets;
        ...
        {
            var window = GetWindowWithRect<GaussianSplatAssetCreator>(new Rect(50, 50, 400, 380), false, "Gaussian Splat Creator", true);
        ...
        void OnGUI()
        {
            ...
            m_GenerateSuccessiveTrainedAlignedAssets = EditorGUILayout.Toggle(new GUIContent("Generate Aligned Assets", 
                "Generate Successive Trained Aligned Assets: When you have successively trained models, " +
                "where for the second model the camera poses were fixed, positional gradients were set to 0 and no more densification has been done."), 
                m_GenerateSuccessiveTrainedAlignedAssets);

            if (m_GenerateSuccessiveTrainedAlignedAssets)
            {
                var rect2 = EditorGUILayout.GetControlRect(true);
                m_InputFile2 = m_FilePicker.PathFieldGUI(rect2, new GUIContent("Input PLY File 2"), m_InputFile2, "ply", "PointCloudFile");
            }
            ...
            if (m_GenerateSuccessiveTrainedAlignedAssets && m_InputFile2 != m_PrevPlyPath2 && !string.IsNullOrWhiteSpace(m_InputFile))
            {
                PLYFileReader.ReadFileHeader(m_InputFile2, out m_PrevVertexCount2, out var _, out var _);
                m_PrevFileSize2 = File.Exists(m_InputFile) ? new FileInfo(m_InputFile).Length : 0;
                m_PrevPlyPath2 = m_InputFile2;
            }
            ...
            if (m_GenerateSuccessiveTrainedAlignedAssets && m_PrevVertexCount2 > 0)
                EditorGUILayout.LabelField("File Size 2", $"{EditorUtility.FormatBytes(m_PrevFileSize2)} - {m_PrevVertexCount2:N0} splats");
            else
                GUILayout.Space(EditorGUIUtility.singleLineHeight);
            ...
            // TODO: check whether the two have the same amount of sizePos
            long sizePos2 = 0, sizeOther2 = 0, sizeCol2 = 0, sizeSHs2 = 0, totalSize2 = 0;
            if (m_GenerateSuccessiveTrainedAlignedAssets && m_PrevVertexCount2 > 0)
            {
                sizePos = GaussianSplatAsset.CalcPosDataSize(m_PrevVertexCount, m_FormatPos);
                sizeOther2 = GaussianSplatAsset.CalcOtherDataSize(m_PrevVertexCount, m_FormatScale);
                sizeCol2 = GaussianSplatAsset.CalcColorDataSize(m_PrevVertexCount, m_FormatColor);
                sizeSHs2 = GaussianSplatAsset.CalcSHDataSize(m_PrevVertexCount, m_FormatSH);
                long sizeChunk = isUsingChunks ? GaussianSplatAsset.CalcChunkDataSize(m_PrevVertexCount) : 0;
                totalSize2 = sizePos2 + sizeOther2 + sizeCol2 + sizeSHs2 + sizeChunk;
            }
            ...
            if (m_GenerateSuccessiveTrainedAlignedAssets)
            {
                if (m_GenerateSuccessiveTrainedAlignedAssets && totalSize2 > 0)
                    EditorGUILayout.LabelField("Asset Size 2",
                        $"{EditorUtility.FormatBytes(totalSize2)} - {(double)m_PrevFileSize2 / totalSize2:F2}x smaller");
                else
                    GUILayout.Space(EditorGUIUtility.singleLineHeight);
            }
        ...
        unsafe void CreateAsset()
        {
            ...
            using NativeArray<InputSplatData> inputSplats2 = m_GenerateSuccessiveTrainedAlignedAssets ? LoadPLYSplatFile(m_InputFile2) : default;
            ...
            ReorderMorton(inputSplats, inputSplats2, boundsMin, boundsMax);

            // cluster SHs
            NativeArray<int> splatSHIndices = default;
            NativeArray<int> splatSHIndices2 = default;
            NativeArray<GaussianSplatAsset.SHTableItemFloat16> clusteredSHs = default;
            NativeArray<GaussianSplatAsset.SHTableItemFloat16> clusteredSHs2 = default;
            if (m_FormatSH >= GaussianSplatAsset.SHFormat.Cluster64k)
            {
                ...
                if(m_GenerateSuccessiveTrainedAlignedAssets)
                    ClusterSHs(inputSplats2, m_FormatSH, out clusteredSHs2, out splatSHIndices2);
            }
            ...
            EditorUtility.DisplayProgressBar(kProgressTitle, "Setup data onto asset 1", 0.95f);
            asset.SetAssetFiles(
            ...
            Debug.Log($"m_GenerateSuccessiveTrainedAlignedAssets = {m_GenerateSuccessiveTrainedAlignedAssets}");
            
            // if flag is set, then we repeat the process for the other file:
            if (m_GenerateSuccessiveTrainedAlignedAssets)
            {
                // set base name
                string baseName2 = Path.GetFileNameWithoutExtension(FilePickerControl.PathToDisplayString(m_InputFile2));
                
                GaussianSplatAsset asset2 = ScriptableObject.CreateInstance<GaussianSplatAsset>();
                asset2.Initialize(inputSplats2.Length, m_FormatPos, m_FormatScale, m_FormatColor, m_FormatSH, boundsMin, boundsMax, cameras);
                asset2.name = baseName2;
               
                var dataHash2 = new Hash128((uint)asset2.splatCount, (uint)asset2.formatVersion, 0, 0);
                
                // second asset, same as for first asset:
                string pathChunk2 = $"{m_OutputFolder}/{baseName2}_chk.bytes";
                string pathPos2 = $"{m_OutputFolder}/{baseName2}_pos.bytes";
                string pathOther2 = $"{m_OutputFolder}/{baseName2}_oth.bytes";
                string pathCol2 = $"{m_OutputFolder}/{baseName2}_col.bytes";
                string pathSh2 = $"{m_OutputFolder}/{baseName2}_shs.bytes";
                LinearizeData(inputSplats2);
                
                if (useChunks)
                    CreateChunkData(inputSplats2, pathChunk2, ref dataHash2);
                CreatePositionsData(inputSplats2, pathPos2, ref dataHash2);
                Debug.Log("CreatePositionsData");
                CreateOtherData(inputSplats2, pathOther2, ref dataHash2, splatSHIndices2);
                Debug.Log("CreateOtherData");
                CreateColorData(inputSplats2, pathCol2, ref dataHash2);
                Debug.Log("CreateColorData");
                CreateSHData(inputSplats2, pathSh2, ref dataHash2, clusteredSHs2);
                Debug.Log("CreateSHData");
                asset2.SetDataHash(dataHash2);
                ...
                EditorUtility.DisplayProgressBar(kProgressTitle, "Setup data onto asset 2", 0.95f);
                asset2.SetAssetFiles(
                    useChunks ? AssetDatabase.LoadAssetAtPath<TextAsset>(pathChunk2) : null,
                    AssetDatabase.LoadAssetAtPath<TextAsset>(pathPos2),
                    AssetDatabase.LoadAssetAtPath<TextAsset>(pathOther2),
                    AssetDatabase.LoadAssetAtPath<TextAsset>(pathCol2),
                    AssetDatabase.LoadAssetAtPath<TextAsset>(pathSh2));
                
                var assetPath2 = $"{m_OutputFolder}/{baseName2}.asset";
                var savedAsset2 = CreateOrReplaceAsset(asset2, assetPath2);
                
                //Selection.activeObject = savedAsset2;
            }
        ...
        static void ReorderMorton(NativeArray<InputSplatData> splatData, NativeArray<InputSplatData> splatData2, float3 boundsMin, float3 boundsMax)
        {
            ...
            // ordering is done only for the first splatData
            ...
            // do not create copy, if splatData2 is not valid
            if (splatData2 == default)
            {
            ...
            {
                NativeArray<InputSplatData> copy2 = new(splatData2, Allocator.TempJob);

                // Reuse the original index for the second splat data.
                for (int i = 0; i < copy.Length; ++i)
                {
                    int originalIndex = order.m_Order[i].Item2;
                    splatData[i] = copy[originalIndex];
                    splatData2[i] = copy2[originalIndex];
                }

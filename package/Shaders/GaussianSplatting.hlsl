...
// computes the rgb data from the spherical harmonics
half3 ShadeSH(SplatSHData splat, half3 dir, int shOrder, bool onlySH, float dimmerSH0,
    float dimmerSH1, float dimmerSH2, float dimmerSH3, float hue, float saturation, float brightness)
{
    dir *= -1;
    ...
    // ambient band
    half3 res = splat.col * dimmerSH0; // col = sh0 * SH_C0 + 0.5 is already precomputed
    if (onlySH)
        res = 0.5;
    // 1st degree
    if (shOrder >= 1)
    {
        res += SH_C1 * (-splat.sh1 * y + splat.sh2 * z - splat.sh3 * x) * dimmerSH1;
        // 2nd degree
        if (shOrder >= 2)
        {
            ...
            res +=
                ...
                (SH_C2[4] * (xx - yy)) * splat.sh8) * dimmerSH2;
            // 3rd degree
            if (shOrder >= 3)
            {
                res +=
                    ...
                    (SH_C3[6] * x * (xx - 3 * yy)) * splat.sh15) * dimmerSH3;
            }
        }
    }

    // hsv adjustment
    float3 hsv_res = RGBtoHSV(res);
    
    hsv_res.x += hue;
    hsv_res.y += saturation;
    hsv_res.z += brightness;
    
    float3 rgb_res = HSVtoRGB(hsv_res);
    
    return max(rgb_res, 0);
}
...
// The actual (raw) data, stored in the Text assets
struct SplatData
{
    float3 pos;
    float4 rot;
...
SplatBufferDataType _SplatPos2;

SplatBufferDataType _SplatOther;
SplatBufferDataType _SplatOther2;

SplatBufferDataType _SplatSH;
SplatBufferDataType _SplatSH2;

Texture2D _SplatColor;
Texture2D _SplatColor2;
uint _SplatFormat;
...
float3 LoadSplatPosValue(uint index, int model=0)
{
    ...
    if(model==0)
        ...
        return LoadAndDecodeVector(_SplatPos2, index * stride, fmt);
}
...
half4 LoadSplatColTex(uint3 coord, int model=0)
{
    if(model==0)
        return _SplatColor.Load(coord);
    else
        return _SplatColor2.Load(coord);
}
// when having 2 assets, model refers to one of them - positions are considered to be fixed
SplatData LoadSplatData(uint idx, int model=0) 
{
    ...
    SplatBufferDataType splatOther;
    SplatBufferDataType splatSH;
    
    if(model == 0)
    {
        splatOther = _SplatOther;
        splatSH = _SplatSH;
    } else
    {
        splatOther = _SplatOther2;
        splatSH = _SplatSH2;
    }

    // load raw splat data, which might be chunk-relative
    s.pos       = LoadSplatPosValue(idx, model);
    s.rot       = DecodeRotation(DecodePacked_10_10_10_2(LoadUInt(splatOther, otherAddr)));
    s.scale     = LoadAndDecodeVector(splatOther, otherAddr + 4, scaleFmt);
    half4 col   = LoadSplatColTex(coord, model);

    uint shIndex = idx;
    if (shFormat > VECTOR_FMT_6)
        shIndex = LoadUShort(splatOther, otherAddr + otherStride - 2);

    uint shOffset = shIndex * shStride;
    uint4 shRaw0 = splatSH.Load4(shOffset);
    uint4 shRaw1 = splatSH.Load4(shOffset + 16);
    if (shFormat == VECTOR_FMT_32F)
    {
        uint4 shRaw2 = splatSH.Load4(shOffset + 32);
        uint4 shRaw3 = splatSH.Load4(shOffset + 48);
        uint4 shRaw4 = splatSH.Load4(shOffset + 64);
        uint4 shRaw5 = splatSH.Load4(shOffset + 80);
        uint4 shRaw6 = splatSH.Load4(shOffset + 96);
        uint4 shRaw7 = splatSH.Load4(shOffset + 112);
        uint4 shRaw8 = splatSH.Load4(shOffset + 128);
        uint4 shRaw9 = splatSH.Load4(shOffset + 144);
        uint4 shRawA = splatSH.Load4(shOffset + 160);
        uint  shRawB = splatSH.Load(shOffset + 176);
    ...
    else if (shFormat == VECTOR_FMT_16 || shFormat > VECTOR_FMT_6)
    {
        ...
        uint4 shRaw3 = splatSH.Load4(shOffset + 48);
        uint4 shRaw4 = splatSH.Load4(shOffset + 64);
        uint3 shRaw5 = splatSH.Load3(shOffset + 80);
    ...
    {
        uint4 shRaw2 = splatSH.Load4(shOffset + 32);
        uint3 shRaw3 = splatSH.Load3(shOffset + 48);
